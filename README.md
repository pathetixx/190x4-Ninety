...
1. **Добавьте источник.** Кнопка **«+»** вверху — вставьте ссылку на подписку (`https://…`) или одиночный конфиг `vless://` / `vmess://` / `trojan://` / `hysteria2://` / `tuic://` из буфера. Подписка сама подтянет список серверов и будет обновляться по своему интервалу.
