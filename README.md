...
- **Два режима подключения** — системный прокси (без прав администратора) и **TUN** через выделенный Windows-сервис: запрос UAC ровно один раз, дальше подключение без него.
...
- **Тонкая настройка** — DNS (remote/direct, fake-DNS), MTU и стек TUN, фрагментация ClientHello и padding, тест соединения и интервалы.
