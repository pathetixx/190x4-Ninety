...
- Portable без пароля сохраняет подписку открытым текстом в
  `NinetyData/config/profile-store.v1`, и она остаётся после перезапуска; после
  задания пароля файл становится envelope, при запуске спрашивается пароль и
  профили читаются после перезапуска/переноса папки на другой тестовый ПК;
  «Убрать пароль» возвращает файлы в открытый текст;
