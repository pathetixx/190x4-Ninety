...
3. (опц.) Прогнать валидацию компиляции без релиза:
   `gh workflow run build.yml --ref main`
4. Тег и push тега — **это запускает релизную сборку**:
   ```
   git tag v0.1.56
   git push origin v0.1.56
   ```
5. **Сразу** создать GitHub Release с заметками — чтобы CI успел вшить их в
   ...
   ```
6. Дождаться зелёного рана (`gh run watch`) и проверить ассеты релиза:
