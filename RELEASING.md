...
`-F`, пушит `main` + тег, создаёт draft-релиз тем же текстом. Останавливается
до компиляции — дальше CI (см. «Что делает CI по тегу»). Перед push тега
спрашивает подтверждение (это уедет пользователям); `--yes` — без вопроса,
`--dry-run` — показать план без записей и сети.
